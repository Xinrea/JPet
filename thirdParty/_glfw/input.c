...
void _glfwInputTray(_GLFWwindow* window, int button, WPARAM w)
{
    if (window->callbacks.trayClick)
        window->callbacks.trayClick((GLFWwindow*)window, button, w);
}
...
//
void _glfwInputDrop(_GLFWwindow* window, int count, const WCHAR** paths)
{
...
GLFWAPI GLFWtrayclickfun glfwSetTrayClickCallback(GLFWwindow* handle,
    GLFWtrayclickfun cbfun)
{
    _GLFWwindow* window = (_GLFWwindow*)handle;
    assert(window != NULL);

    _GLFW_REQUIRE_INIT_OR_RETURN(NULL);
    _GLFW_SWAP_POINTERS(window->callbacks.trayClick, cbfun);
    return cbfun;
}
