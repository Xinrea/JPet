...
static void updateCursorImage(_GLFWwindow* window) {
  if (window->cursorMode == GLFW_CURSOR_NORMAL ||
    ...
    // HACK: When running locally, it is set to NULL, but when connected via
    // Remote
...
static LRESULT CALLBACK windowProc(HWND hWnd, UINT uMsg, WPARAM wParam,
  ...
  switch (uMsg) {
    case WM_IAWENTRAY: {
      switch (lParam) {
        case WM_LBUTTONDOWN: {
          _glfwInputTray(window, 1, wParam);
          break;
        }
        case WM_RBUTTONDOWN: {
          _glfwInputTray(window, 2, wParam);
          break;
        }
      }
      break;
    ...
    case WM_COMMAND: {
      _glfwInputTray(window, 0, wParam);
      break;
    }
    ...
    case WM_DPICHANGED: {
      ...
      if (!window->monitor && (window->win32.scaleToMonitor ||
...
static int createNativeWindow(_GLFWwindow* window,
  ...
  if (!_glfw.win32.mainWindowClass) {
    ...
    wc.hIcon = LoadImageW(GetModuleHandleW(NULL), L"GLFW_ICON", IMAGE_ICON, 0,
  ...
  if (GetSystemMetrics(SM_REMOTESESSION)) {
    ...
    //       When not on Remote Desktop, this handle is NULL and normal hiding
    //       is used
  ...
  window->win32.handle =
      CreateWindowExW(exStyle, MAKEINTATOM(_glfw.win32.mainWindowClass),
...
void _glfwGetFramebufferSizeWin32(_GLFWwindow* window, int* width,
                                  int* height) {
...
void _glfwSetWindowMousePassthroughWin32(_GLFWwindow* window,
                                         GLFWbool enabled) {
  COLORREF key = 0;
  BYTE alpha = 0;
  DWORD flags = 0;
...
void _glfwPollEventsWin32(void) {
  ...
  if (window) {
    ...
    // The re-center is required in order to prevent the mouse cursor stopping
