...
GLFWAPI void glfwSetInputMode(GLFWwindow* handle, int mode, int value) {
  ...
  switch (mode) {
    case GLFW_CURSOR: {
      ...
      _glfw.platform.getCursorPos(window, &window->virtualCursorPosX,
...
GLFWAPI GLFWtrayfun glfwSetWindowTrayCallback(GLFWwindow* handle,
                                              GLFWtrayfun cbfun) {
  _GLFWwindow* window = (_GLFWwindow*)handle;
  assert(window != NULL);

  _GLFW_REQUIRE_INIT_OR_RETURN(NULL);
  _GLFW_SWAP(GLFWtrayfun, window->callbacks.tray, cbfun);
  return cbfun;
}
        ...
        if (parseMapping(&mapping, line)) {
          ...
          if (previous)
            *previous = mapping;
            ...
            _glfw.mappingCount++;
            _glfw.mappings = _glfw_realloc(
                _glfw.mappings, sizeof(_GLFWmapping) * _glfw.mappingCount);
