...
void LAppLive2DManager::OnTap(csmFloat32 x, csmFloat32 y) {
  ...
  for (csmUint32 i = 0; i < _models.GetSize(); i++) {
    if (_models[i]->HitTest(HitAreaNameChange, x, y)) {
      ...
      if (hr != InvalidMotionQueueEntryHandleValue) {
        AudioManager::GetInstance()->Play3dSound(
            string("resources/audios/") + (_editMode ? "d02.mp3" : "d01.mp3"));
        _editMode = !_editMode;
      }
    ...
    } else if (_models[i]->HitTest(HitAreaNameArmL, x, y) ||
               _models[i]->HitTest(HitAreaNameArmR, x, y)) {
      if (DebugLogEnable) {
      ...
      if (_editMode) {
        ...
        if (hr != InvalidMotionQueueEntryHandleValue) _legBeltCount++;
               ...
               _models[i]->HitTest(HitAreaNameLegs, x, y))  // 旧衣服的腿
    {
      if (DebugLogEnable) {
      ...
      if (_editMode) {
        ...
        PlayTouchAudio("t04.mp3");
               ...
               _models[i]->HitTest(HitAreaNameLegs, x, y))  // 新衣服的腿
    {
      if (DebugLogEnable) {
        LAppPal::PrintLog("[APP]hit area: [%s]", HitAreaNameLegs);
      }
